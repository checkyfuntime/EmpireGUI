...
namespace EmpireGUI
{
    ...
    {
        //Variables that are needed throughout random places but only in frmMain.cs
        ...
        public void Form1_Load(object sender, EventArgs e)
        {
            ...
            bgUpdate.ProgressChanged += new ProgressChangedEventHandler(bgUpdate_ProgressChanged);
        ...
        //Functions that draw on the main form.
        ...
        //Functions that fill Controls with values
        private void updateValues()
        {
            ...
            {
                if (!lstListeners.Items.Contains(subject.name))
                {
                    lstListeners.Items.Add(subject.name);
                }
            }
            if (Globals.agentsJSON != null)
            {
                try
                {
                    ...
                    if (agents != null)
                    {
                        foreach (var agent in agents)
                        {
                            ...
                            if (!lstAgents.Items.Contains(agent["name"]))
                            {
        ...
        private void fillValues()
        {
            ...
            if (Globals.agentsJSON != null)
            {
                ...
                {
                    displayCustomAlert("Couldn't load Agents! (JsonReaderException)");
                }
            }
        ...
        private string killAgent(string agentName)
        {
            ...
            if(!string.IsNullOrEmpty(results))
            {
                ...
                {
                    return "An error occured and listeners could not be killed";
                }
            }
            else
            {
        ...
        {
            cmbListenerSelect.SelectedIndex = -1;
            ...
            //Get Listbox Tokens
            //Would a try/catch work better in these situations?
            if (Globals.listenersJSON != null && lstListeners.SelectedItem != null)
            {
        ...
        private void btnNewListener_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(selectedListener))
            {
                displayCustomAlert("Select a Listener Type!");
                return;
            }
        ...
        {
            foreach (Control control in ctrlListeners.Controls)
            {
                if (control is MetroTextBox)
                {
                    control.Text = "";
        ...
        private void btnkillAllListeners_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(results))
                {
                    ...
                    {
                        displayCustomAlert("Could Not Remove Listeners (MSGFAIL)");
                    }
                }
                else
                {
                    displayCustomAlert("Could Not Remove Listeners (NO RESPONSE)");
                }
            }
            catch
            {
        ...
        private void cmbListenerSelect_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cmbListenerSelect.SelectedItem != null)
            {
        ...
        public static void displayCustomAlert(string Message)
        {
        ...
        private void handleWebException(WebException ex)
        {
            if (ex.Status == WebExceptionStatus.ProtocolError && ex.Response != null)
            {
                ...
                switch (webresponse.StatusCode)
                {
                    ...
                    case HttpStatusCode.BadRequest:
                        displayCustomAlert("Bad Request (400) Received from the Empire Server. Are all the appropriate options set?");
                        break;
            ...
            {
                displayCustomAlert("An error occured when contacting Empire Server (" + ex.Message + ")");
            }
        }
    ...
    rtj - ?";
        ...
        {
            /** 
             * Since there are no WebSockets available to call it, wait the sleeptime + jittertime and then try to pull the results back from the server.
             **/
            ...
            if (resultsJSON != null)
            {
                ...
                e.Result = results[0]["AgentResults"][0].ToString().Replace("\\r\\n", Environment.NewLine);
            }
            else
            {
                txtConsoleLog.AppendText("[-] Couldn't run shell command");
                agentConsoles[lstAgents.SelectedItem.ToString().Replace(" (STALE)", "")].Add("[-] Couldn't run shell command");
            }
        }
        private void bgw_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            {
                //Nothing happened. Should I appened to textbox or just not return?
            }
        }
        private void bgUpdate_DoWork(object sender, DoWorkEventArgs e)
        {
            while (true)
            {
                Thread.Sleep(Globals.pollingValue * 1000);
                getAgents();
                getListeners();
                syncCreds();
        ...
        private void bgUpdate_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            updateValues();
        }
            ...
            try
            {
                if (!string.IsNullOrEmpty(results))
                {
                    ...
                    if (token["success"] != null)
                    {
                        ...
                        {
                            string clearfail = "[-] Error occured when clearing taskings";
                            txtConsoleLog.AppendText(clearfail + Environment.NewLine);
                            agentConsoles[agentName].Add(clearfail);
                        }
                    }
                    else
                    {
